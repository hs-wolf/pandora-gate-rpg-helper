...
import { Attributes, EffectFieldsList } from '@pandora-gate-rpg-helper/models';
...
const saveEdits = () => {
  charactersStore.updateCharacter(currentCharacter.value?.id ?? '', {
    attributes: currentCharacter.value?.attributes,
  });
  editing.value = false;
};
</script>
...
<template>
  ...
  >
    ...
    >
      <p>{{ t('characters-attributes-form.title') }}</p>
    ...
    <div v-if="showForm" class="flex flex-col gap-4">
      <div v-if="editing" class="flex flex-col gap-4">
        ...
        >
          ...
          <input
            type="number"
            v-model="currentCharacter.attributes[field as keyof Attributes]"
            class="form-input"
          />
          <p class="ml-4">
        ...
        >
          <div class="col-span-2 display-section">
            ...
            <span class="text-sm font-normal">
              ({{ currentCharacter.attributes[field as keyof Attributes] }})
            </span>
            <p>
              {{
                currentCharacter.getFieldFinalValue(field as EffectFieldsList)
          ...
          <div class="display-section">
            <h1 class="text-sm">
              {{ t('characters-attributes-form.modificator.acronym') }}
            </h1>
            <p>
              {{ currentCharacter.getAttributeMod(field as keyof Attributes) }}
            </p>
          </div>
        </div>
        <button
