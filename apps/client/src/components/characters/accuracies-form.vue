...
const editableAccuracies = computed(() => props.character.accuracies);
  ...
  >
    ...
    >
      <p class="text-sm font-semibold">
        {{ t('characters-accuracies-form.title') }}
      </p>
    ...
    <div v-if="showData" class="flex flex-col p-4 gap-4">
      <div class="flex flex-col gap-2">
        <div
          v-for="(key, index) in Object.keys(editableAccuracies)"
          :key="index"
          class="item-simple"
        >
          <h1 class="col-span-2">
            {{ t(`characters-accuracies-form.${key}`) }}
          </h1>
          <p v-if="editing" class="col-span-2 flex items-center gap-4">
            <input
              type="number"
              v-model="editableAccuracies[key as keyof Accuracies]"
          ...
          <p v-else class="col-span-2 flex items-center gap-2">
            <span class="text-sm font-normal">
              ({{ editableAccuracies[key as keyof Accuracies] }})
            </span>
            <span>
              {{ character.getFieldFinalValue(key as EffectFieldsList) }}
            </span>
