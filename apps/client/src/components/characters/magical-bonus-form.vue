...
const saveEdits = () => {
  charactersStore.updateCharacter(currentCharacter.value?.id ?? '', {
    magicalBonus: currentCharacter.value?.magicalBonus,
  });
  editing.value = false;
};
</script>
...
<template>
  ...
  >
    ...
    >
      <p>{{ t('characters-magical-bonus-form.title') }}</p>
    ...
    <div v-if="showForm" class="flex flex-col gap-4">
      <div v-if="editing" class="flex flex-col gap-4">
        <div
          v-for="field in Object.keys(currentCharacter.magicalBonus).sort(
            (a, b) =>
              t(`characters-magical-bonus-form.${a}`) <
              t(`characters-magical-bonus-form.${b}`)
                ? -1
                : 1
          )"
          :key="field"
          class="form-section"
        >
          <h1>{{ t(`characters-magical-bonus-form.${field}`) }}</h1>
          <input
            type="number"
            v-model="currentCharacter.magicalBonus[field as keyof MagicalBonus]"
            class="form-input"
          />
          <p class="ml-4">
      ...
      <div v-else class="flex flex-col gap-4">
        ...
        >
          ...
          <span class="text-sm font-normal">
            ({{ currentCharacter.magicalBonus[field as keyof MagicalBonus] }})
          </span>
          <p>
            {{ currentCharacter.getFieldFinalValue(field as EffectFieldsList) }}
          </p>
