...
const editablePhysicalBonus = computed(() => props.character.physicalBonus);
  ...
  >
    ...
    >
      <p class="text-sm font-semibold">
        {{ t('characters-physical-bonus-form.title') }}
      </p>
    ...
    <div v-if="showData" class="flex flex-col p-4 gap-4">
      <div class="flex flex-col gap-2">
        <div
          v-for="(key, index) in Object.keys(editablePhysicalBonus)"
          :key="index"
          class="item-simple"
        >
          <h1 class="col-span-2">
            {{ t(`characters-physical-bonus-form.${key}`) }}
          </h1>
          <p v-if="editing" class="col-span-2 flex items-center gap-4">
            <input
              type="number"
              v-model="editablePhysicalBonus[key as keyof PhysicalBonus]"
          ...
          <p v-else class="col-span-2 flex items-center gap-2">
            <span class="text-sm font-normal">
              ({{ editablePhysicalBonus[key as keyof PhysicalBonus] }})
            </span>
            <span>
              {{ character.getFieldFinalValue(key as EffectFieldsList) }}
            </span>
