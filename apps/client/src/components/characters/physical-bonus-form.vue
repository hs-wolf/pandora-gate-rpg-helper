...
const saveEdits = () => {
  charactersStore.updateCharacter(currentCharacter.value?.id ?? '', {
    physicalBonus: currentCharacter.value?.physicalBonus,
  });
  editing.value = false;
};
</script>
...
<template>
  ...
  >
    ...
    >
      <p>{{ t('characters-physical-bonus-form.title') }}</p>
    ...
    <div v-if="showForm" class="flex flex-col gap-4">
      <div v-if="editing" class="flex flex-col gap-4">
        <div
          ...
          class="form-section"
        >
          <h1>{{ t(`characters-physical-bonus-form.${field}`) }}</h1>
          <input
            type="number"
            v-model="currentCharacter.physicalBonus[field as keyof PhysicalBonus]"
            class="form-input"
          />
          <p class="ml-4">
      ...
      <div v-else class="flex flex-col gap-4">
        ...
        >
          ...
          <span class="text-sm font-normal">
            ({{ currentCharacter.physicalBonus[field as keyof PhysicalBonus] }})
          </span>
          <p>
            {{ currentCharacter.getFieldFinalValue(field as EffectFieldsList) }}
          </p>
